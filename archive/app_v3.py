...
<style>
    ...
    .exec-brief {
        max-width: 1200px; margin: 20px auto 0 auto; padding: 20px 40px;
        background: #F8F9FA; border-radius: 12px; border: 1px solid #E9ECEF;
        font-size: 1.1rem; color: #1D1D1F; line-height: 1.6; text-align: center;
    }
...
# ---------------------------------------------------------
# 2. DATA ENGINE & STATISTICAL BOUNDS
# ---------------------------------------------------------
...
# Calculate Statistical Normalcy for Acceleration
accel_std = view_df['acceleration'].std()
accel_mean = view_df['acceleration'].mean()
upper_limit = accel_mean + (2 * accel_std) # 95% Confidence threshold
lower_limit = accel_mean - (2 * accel_std)
...
is_anomaly = abs(accel) > upper_limit # Detection logic
...
<div class="dashboard-header">
    ...
    <div class="exec-brief">
        ...
        Analysis confirms a <b>{status_brief.lower()}</b> trend. 
...
with k2:
    # Color logic: Turns bright red ONLY if anomaly detected
    trend_color = "#FF3B30" if accel > 0 else "#28A745"
    if is_anomaly: trend_color = "#D70015" # Deeper red for statistical anomalies
    st.markdown(f"""<div class="metric-box"><div class="m-label">Risk Trajectory</div><div class="m-value" style="color:{trend_color}">{'↑' if accel > 0 else '↓'} {abs(accel):.2f}</div>
    <div class="m-context"><b>{status_brief}</b><br>{'ALARM: Statistical Anomaly' if is_anomaly else 'Daily rate of harm accumulation.'}</div></div>""", unsafe_allow_html=True)
with k3:
...
# ---------------------------------------------------------
# 5. SYNCHRONIZED VISUALIZATION CONTAINERS
# ---------------------------------------------------------
...
with col_l:
    with st.container(border=True):
        ...
        # ACCELERATION WITH CONTROL BOUNDS
        fig_a = go.Figure()
        # Shading for Normal Variance zone
        fig_a.add_hrect(y0=lower_limit, y1=upper_limit, fillcolor="rgba(134, 134, 139, 0.05)", line_width=0, layer="below")
        fig_a.add_hline(y=upper_limit, line_dash="dot", line_color="rgba(255, 59, 48, 0.3)")
        fig_a.add_hline(y=0, line_color="#86868B", line_width=1)
        
        fig_a.add_trace(go.Scatter(x=view_df["Date"], y=view_df["acceleration"], 
                                   line=dict(color="#FF3B30", width=2), fill='tonexty', 
                                   fillcolor="rgba(255, 59, 48, 0.05)"))
        
        fig_a.update_layout(title="<b>KINETIC MOMENTUM (ACCELERATION)</b>", height=250, margin=dict(l=60, r=20, t=40, b=40), paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', showlegend=False)
